...
const ComplianceCenter = () => {
  ...
  const data = selectedAccount.name == 'Governance' ? useSelector(state => state.compTableItems.data) : useSelector(state => state.compTableItems.data2) 
  ...
  const [remediation, setRemediation] = useState(['Available', 'Unavailable']);
  const [selectedRemediations, setSelectedRemediations] = useState(new Set(['AVAILABLE', 'UNAVAILABLE']));
    ...
    <div className={`h-[91.3vh] flex-1 overflow-y-auto ${!isNavOpen ? 'ml-16' : 'ml-48'} px-4 py-2 ${isBotopen ? 'mr-64' : ' md:px-14'} ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <div className={`flex flex-wrap justify-between mt-3 py-4 border-2 ${darkMode ? 'border-blue-100' : 'border-blue-700'} rounded-md px-4 hover:outline hover:outline-blue-500`}>
        <div className='relative flex gap-2 items-center'>
          <h3 className='text-lg font-bold mr-4'><button onClick={() => setSeverity('')}>Compliance Center</button></h3>
        </div>
                ...
                {isTriggerOpen && (
                  <ul ref={filterRef} className='absolute z-10 text-black bg-white border border-blue-300 rounded-md mt-3 w-full max-h-36 overflow-y-auto text-left'>
                    {['Configuration Change', 'Periodic, 24 Hours'].map((item, index) => (
                      <li
                        key={index}
                        className={`px-2 py-1 flex items-center gap-2 text-sm font-sans ${index % 2 !== 0 ? 'bg-gray-200' : ''}`}
                      >
          ...
          {filteredRules.map((item) => (
            <tr key={item.id} className="mb-2">
              <td className={`px-4 py-2 text-left ${darkMode ? 'border-blue-100' : 'border-blue-700'}`}>
                <Link to={`/standards/compliance/${item.id}`} className="text-blue-500 ">
                  {item.name}
                </Link>
              </td>
              <td className={`px-4 py-2 text-center ${darkMode ? 'border-blue-100' : 'border-blue-700'}`}><span className={`mr-1 ${item.autoRemediation == 'Available' ? 'pr-4': 'pl-0.5'}`}><CircleIcon style={{height:'17px', marginRight: '5px', width:'17px', color: item.autoRemediation == 'Available' ? 'green' : '#CF171F'}}/></span>{item.autoRemediation}</td>
